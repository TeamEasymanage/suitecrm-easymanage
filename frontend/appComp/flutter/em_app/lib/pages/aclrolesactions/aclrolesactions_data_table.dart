...
class AclRolesActionsDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AclRolesActionsDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AclRolesActionsDataTableState createState() => _AclRolesActionsDataTableState(); 
} 
 
class _AclRolesActionsDataTableState extends State<AclRolesActionsDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AclRolesActions Data Table"; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AclRolesActions List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AclRolesActions List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AclRolesActionsSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AclRolesActionsService.AclRolesActionsSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : AclRolesActionsViewAll ... "); 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AclRolesActionsFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AclRolesActionsService.AclRolesActionsFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AclRolesActionsDataSource(aclRolesActionsList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                ...
                DataColumn( 
                    label: const Text('Action Id'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Access Override'), onSort: doColumnSort), 
                DataColumn( 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AclRolesActionsCardList = 
        aclRolesActionsList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AclRolesActionsCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AclRolesActions e) { 
...
Widget _buildListView() {  
    ...
    if (aclRolesActionsList.length == 0) { 
    return Column( 
      children: [ 
    ...
    var noOfFields = aclRolesActionsList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            aclRolesActionsList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(aclRolesActionsList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(aclRolesActionsList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: aclRolesActionsList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AclRolesActions oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AclRolesActions Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AclRolesActionsInfo(foundAclRolesActions: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AclRolesActionsDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AclRolesActionsEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AclRolesActionsService.AclRolesActionsDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  aclRolesActionsList.sort((item1, item2) => 
	  ...
	  aclRolesActionsList.sort((item1, item2) => 
		  compareColumn(ascending, item1.actionId, item2.actionId)); 
	} else if (columnIndex == 3) { 
	  aclRolesActionsList.sort((item1, item2) => 
		  compareColumn(ascending, item1.accessOverride, item2.accessOverride)); 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AclRolesActionsDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AclRolesActionsDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AclRolesActions> _data; 
 
  AclRolesActionsDataSource(List<AclRolesActions> aclRolesActionsList) { 
    _data = aclRolesActionsList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].actionId))), 
	DataCell(Text(getEmStrOpt(getEmFmtNumOpt(_data[index].accessOverride)))), 
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AclRolesActionsInfo(foundAclRolesActions: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AclRolesActionsDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AclRolesActionsInfo extends StatelessWidget { 	
	  final AclRolesActions foundAclRolesActions; 	
	  const _AclRolesActionsInfo({Key? key, required this.foundAclRolesActions}) 	
	      : super(key: key); 	
