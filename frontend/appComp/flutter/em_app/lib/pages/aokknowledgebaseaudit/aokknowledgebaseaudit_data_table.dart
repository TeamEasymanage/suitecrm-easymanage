...
import 'package:em_app/pages/aokknowledgebaseaudit/aokknowledgebaseaudit_service.dart'; 
import 'package:em_app/pages/aokknowledgebaseaudit/aokknowledgebaseaudit_lib.dart'; 
...
class AokKnowledgebaseAuditDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AokKnowledgebaseAuditDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AokKnowledgebaseAuditDataTableState createState() => _AokKnowledgebaseAuditDataTableState(); 
} 
 
class _AokKnowledgebaseAuditDataTableState extends State<AokKnowledgebaseAuditDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AokKnowledgebaseAudit Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<AokKnowledgebaseAudit> aokKnowledgebaseAuditList = []; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AokKnowledgebaseAudit List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AokKnowledgebaseAudit List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AokKnowledgebaseAuditSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AokKnowledgebaseAuditService.AokKnowledgebaseAuditSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
      ...
      setState(() { 
        aokKnowledgebaseAuditList = getList; 
        _loading = false; 
      }); 
      }  
    }); 
    ...
    print("Calling Service : AokKnowledgebaseAuditViewAll ... "); 
    AokKnowledgebaseAuditService.AokKnowledgebaseAuditViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AokKnowledgebaseAuditFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AokKnowledgebaseAuditService.AokKnowledgebaseAuditFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AokKnowledgebaseAuditDataSource(aokKnowledgebaseAuditList), 
                      columns: <DataColumn>[ 
                DataColumn( 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AokKnowledgebaseAuditCardList = 
        aokKnowledgebaseAuditList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AokKnowledgebaseAuditCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AokKnowledgebaseAudit e) { 
...
Widget _buildListView() {  
    ...
    if (aokKnowledgebaseAuditList.length == 0) { 
    return Column( 
      children: [ 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) =>  
                                  const AokKnowledgebaseAuditForm(), 	 
                                  ), 	 
                                ); 	 
                              }, 	 
                              icon: const Icon( 	 
    ...
    var noOfFields = aokKnowledgebaseAuditList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            aokKnowledgebaseAuditList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(aokKnowledgebaseAuditList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(aokKnowledgebaseAuditList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: aokKnowledgebaseAuditList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AokKnowledgebaseAudit oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AokKnowledgebaseAudit Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AokKnowledgebaseAuditInfo(foundAokKnowledgebaseAudit: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AokKnowledgebaseAuditDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AokKnowledgebaseAuditEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AokKnowledgebaseAuditService.AokKnowledgebaseAuditDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  aokKnowledgebaseAuditList.sort((item1, item2) => 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AokKnowledgebaseAuditDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AokKnowledgebaseAuditDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AokKnowledgebaseAudit> _data; 
 
  AokKnowledgebaseAuditDataSource(List<AokKnowledgebaseAudit> aokKnowledgebaseAuditList) { 
    _data = aokKnowledgebaseAuditList; 
  } 
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AokKnowledgebaseAuditInfo(foundAokKnowledgebaseAudit: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AokKnowledgebaseAuditDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AokKnowledgebaseAuditInfo extends StatelessWidget { 	
	  final AokKnowledgebaseAudit foundAokKnowledgebaseAudit; 	
	  const _AokKnowledgebaseAuditInfo({Key? key, required this.foundAokKnowledgebaseAudit}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AokKnowledgebaseAudit_dispForm(context, foundAokKnowledgebaseAudit); 	
	  } 	
	} 	
