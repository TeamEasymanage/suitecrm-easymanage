...
class AddressBookDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AddressBookDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AddressBookDataTableState createState() => _AddressBookDataTableState(); 
} 
 
class _AddressBookDataTableState extends State<AddressBookDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AddressBook Data Table"; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AddressBook List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AddressBook List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AddressBookSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AddressBookService.AddressBookSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : AddressBookViewAll ... "); 
    AddressBookService.AddressBookViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AddressBookFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AddressBookService.AddressBookFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AddressBookDataSource(addressBookList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                    label: const Text('Assigned User Id'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Bean'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Bean Id'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AddressBookCardList = 
        addressBookList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AddressBookCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AddressBook e) { 
...
Widget _buildListView() {  
    ...
    var noOfFields = addressBookList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            addressBookList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(addressBookList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(addressBookList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: addressBookList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AddressBook oh) { 
                        ...
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AddressBook Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AddressBookInfo(foundAddressBook: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  content: Container( 
                                    child: Column( 
                                      mainAxisSize: MainAxisSize.min, 
                                      children: [ 
                                        Text("Record Id: " +oh.assignedUserId.toString() ), 
                                        const Text(""), 
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      onPressed: () { 
                                        String valParam = ""; 
                                        if (oh.assignedUserId is String ) { 
                                          valParam = "'"+oh.assignedUserId.toString()+"'"; 
                                        } else { 
                                          valParam = oh.assignedUserId.toString(); 
                                        } 
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AddressBookDataTable(viewType: dispType, pageNo: 0, searchClause: " assignedUserId = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                              ...
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AddressBookEdit( 	 
                      				assignedUserIdSelected: oh.assignedUserId, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                              icon: const Icon( 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AddressBookService.AddressBookDelete( 	 
                                      oh.assignedUserId) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  addressBookList.sort((item1, item2) => 
		  compareColumn(ascending, item1.assignedUserId, item2.assignedUserId)); 
	} else if (columnIndex == 1) { 
	  addressBookList.sort((item1, item2) => 
		  compareColumn(ascending, item1.bean, item2.bean)); 
	} else if (columnIndex == 2) { 
	  addressBookList.sort((item1, item2) => 
		  compareColumn(ascending, item1.beanId, item2.beanId)); 
    } 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AddressBookDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AddressBookDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AddressBook> _data; 
 
  AddressBookDataSource(List<AddressBook> addressBookList) { 
    _data = addressBookList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].bean))), 
	DataCell(Text(getEmStrOpt(_data[index].beanId))), 
      DataCell(Row( 	
        mainAxisAlignment: MainAxisAlignment.spaceBetween, 	
        children: [ 	
          Builder(builder: (context) { 	
            return IconButton( 	
                onPressed: () { 	
                  Navigator.of(context).push(MaterialPageRoute( 	
                    builder: (context) => AddressBookEdit( 	
                      assignedUserIdSelected: _data[index].assignedUserId, 	
                    ), 	
                  )); 	
                }, 	
                icon: const Icon( 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AddressBookInfo(foundAddressBook: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AddressBookDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AddressBookInfo extends StatelessWidget { 	
	  final AddressBook foundAddressBook; 	
	  const _AddressBookInfo({Key? key, required this.foundAddressBook}) 	
	      : super(key: key); 	
