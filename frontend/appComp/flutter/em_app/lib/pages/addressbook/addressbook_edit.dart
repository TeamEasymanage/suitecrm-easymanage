...
class AddressBookEdit extends StatefulWidget { 
  ...
  final String assignedUserIdSelected; 
  const AddressBookEdit({Key? key, required this.assignedUserIdSelected}) : super(key: key); 
...
class _AddressBookEditState extends State<AddressBookEdit> { 
  ...
  //List<String> _assignedUserIds = ["Select A Key Value"]; 	
  String _chosenValue = ""; 	
  ...
  void initState() {	
    ...
    _chosenValue = widget.assignedUserIdSelected.toString();	
    _loading = true;	
    ...
    //Key Value passed, Edit Record Mode 	
		...
		_loading = false;	
    /*	
    AddressBookService.AddressBookViewAllIds().then((ids) { 	
      if (ids is String) {	
        setState(() {	
          errorMsg = ids;	
        });	
      ...
      setState(() { 	
        _assignedUserIds = ids; 	
        _assignedUserIds.insert(0, "Select A Key Value");	
		 //Remove duplicates 	
        _assignedUserIds = _assignedUserIds.toSet().toList();	
        _loading = false;	
      });	
    });	
    */	
  ...
  Widget build(BuildContext context) { 	
    return Scaffold( 	
        ...
        appBar: AppBar( 	
          title: const Text("AddressBook Edit  Form"), 	
          centerTitle: true, 	
        ), 	
        body: _loading 	
                    ...
                    Padding(	
                      ...
                      _isKeySelected 	
                          ? FutureBuilder<AddressBook>( 	
                              future: AddressBookService 	
                                      .AddressBookQuery(_chosenValue) 	
                                  .then((value) => value), 	
                                ...
                                switch (snapshot.connectionState) { 	
                                  ...
                                  default: 	
                                    if (snapshot.hasError) { 	
                                      ...
                                      return AddressBookEditRec( 	
                                          addressBook: snapshot.data!); 	
                                    } 	
                                } 	
                              }) 	
                          : const SizedBox( 	
...
class AddressBookEditRec extends StatefulWidget { 
  final AddressBook addressBook; 
  const AddressBookEditRec({Key? key, required this.addressBook}) : super(key: key); 
 
  @override 
  _AddressBookEditRecState createState() => _AddressBookEditRecState(); 
} 
 
class _AddressBookEditRecState extends State<AddressBookEditRec> { 
  AddressBook foundAddressBook = AddressBook(assignedUserId: "" ); 
...
void didChangeDependencies() { 
  ...
  void initState() { 
    super.initState(); 
    setState(() { 
      foundAddressBook = widget.addressBook; 
    }); 
  } 
           ...
           child: SingleChildScrollView( 
            child: Form( 
              ...
              child: Padding( 
                ...
                children: [ 
				  // ---------------------------------------------------------------------------------------  
				  // Column Details:  Srl No : [1] [ Name [Assigneduserid] : EM Type [CHAR], Java Type [String]  
				...
				Flexible( 
					  child:  
				  TextFormField( 
					   enabled: false,    initialValue: getEmStrOpt(foundAddressBook.assignedUserId),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
				  ...
				  // ---------------------------------------------------------------------------------------  
				  // Column Details:  Srl No : [2] [ Name [Bean] : EM Type [CHAR], Java Type [String]  
				  //                  EM size: maxlength: 50, Display Size : 20 
				  // Flutter Details: Field Label [Bean], Name [bean], Type [String]  
				  // ---------------------------------------------------------------------------------------  
				Row( 
				  mainAxisAlignment: MainAxisAlignment.spaceBetween, 
				  children: <Widget>[ 
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAddressBook.bean),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
					...
					*/  
					//maxLength: 30, 
					onSaved: (value) => setState(() => beanVar = value.toString()), 
				  ), //textformfield 
				), //flexible 
				]), //Widget[] //Row 
                  const SizedBox(height: 10), 
				  // ---------------------------------------------------------------------------------------  
				  // Column Details:  Srl No : [3] [ Name [Beanid] : EM Type [CHAR], Java Type [String]  
				  //                  EM size: maxlength: 36, Display Size : 20 
				  // Flutter Details: Field Label [Bean Id], Name [beanId], Type [String]  
				  // ---------------------------------------------------------------------------------------  
				Row( 
				  mainAxisAlignment: MainAxisAlignment.spaceBetween, 
				  children: <Widget>[ 
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAddressBook.beanId),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
					...
					*/  
					//maxLength: 30, 
					onSaved: (value) => setState(() => beanIdVar = value.toString()), 
				  ), //textformfield 
				), //flexible 
				]), //Widget[] //Row 
                  const SizedBox(height: 10), 
                    ...
                    builder: (context) => ButtonWidget( 
                      ...
                      onClicked: () async { 
					   ...
					   beanId: beanIdVar , 
                          ...
                          await AddressBookService.AddressBookEdit( 
                                  newAddressBook.assignedUserId, 
                                  newAddressBook) 
                              .then((message) { 
                                    apiMessage = message; 
