...
class AmProjecttemplatesDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AmProjecttemplatesDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AmProjecttemplatesDataTableState createState() => _AmProjecttemplatesDataTableState(); 
} 
 
class _AmProjecttemplatesDataTableState extends State<AmProjecttemplatesDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AmProjecttemplates Data Table"; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AmProjecttemplates List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AmProjecttemplates List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AmProjecttemplatesSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AmProjecttemplatesService.AmProjecttemplatesSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : AmProjecttemplatesViewAll ... "); 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AmProjecttemplatesFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AmProjecttemplatesService.AmProjecttemplatesFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AmProjecttemplatesDataSource(amProjecttemplatesList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                ...
                DataColumn( 
                    label: const Text('Override Business Hours'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AmProjecttemplatesCardList = 
        amProjecttemplatesList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AmProjecttemplatesCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AmProjecttemplates e) { 
...
Widget _buildListView() {  
    ...
    if (amProjecttemplatesList.length == 0) { 
    return Column( 
      children: [ 
    ...
    var noOfFields = amProjecttemplatesList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            amProjecttemplatesList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(amProjecttemplatesList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(amProjecttemplatesList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: amProjecttemplatesList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AmProjecttemplates oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AmProjecttemplates Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AmProjecttemplatesInfo(foundAmProjecttemplates: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AmProjecttemplatesDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AmProjecttemplatesEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AmProjecttemplatesService.AmProjecttemplatesDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  amProjecttemplatesList.sort((item1, item2) => 
	  ...
	  amProjecttemplatesList.sort((item1, item2) => 
		  compareColumn(ascending, item1.overrideBusinessHours, item2.overrideBusinessHours)); 
    } 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AmProjecttemplatesDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AmProjecttemplatesDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AmProjecttemplates> _data; 
 
  AmProjecttemplatesDataSource(List<AmProjecttemplates> amProjecttemplatesList) { 
    _data = amProjecttemplatesList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].overrideBusinessHours))), 
      DataCell(Row( 	
        mainAxisAlignment: MainAxisAlignment.spaceBetween, 	
        children: [ 	
          Builder(builder: (context) { 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AmProjecttemplatesInfo(foundAmProjecttemplates: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AmProjecttemplatesDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AmProjecttemplatesInfo extends StatelessWidget { 	
	  final AmProjecttemplates foundAmProjecttemplates; 	
	  const _AmProjecttemplatesInfo({Key? key, required this.foundAmProjecttemplates}) 	
	      : super(key: key); 	
