...
import 'package:em_app/pages/aopcaseeventsaudit/aopcaseeventsaudit_service.dart'; 
import 'package:em_app/pages/aopcaseeventsaudit/aopcaseeventsaudit_lib.dart'; 
...
class AopCaseEventsAuditDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AopCaseEventsAuditDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AopCaseEventsAuditDataTableState createState() => _AopCaseEventsAuditDataTableState(); 
} 
 
class _AopCaseEventsAuditDataTableState extends State<AopCaseEventsAuditDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AopCaseEventsAudit Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<AopCaseEventsAudit> aopCaseEventsAuditList = []; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AopCaseEventsAudit List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AopCaseEventsAudit List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AopCaseEventsAuditSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AopCaseEventsAuditService.AopCaseEventsAuditSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
      ...
      setState(() { 
        aopCaseEventsAuditList = getList; 
        _loading = false; 
      }); 
      }  
    }); 
    ...
    print("Calling Service : AopCaseEventsAuditViewAll ... "); 
    AopCaseEventsAuditService.AopCaseEventsAuditViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AopCaseEventsAuditFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AopCaseEventsAuditService.AopCaseEventsAuditFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AopCaseEventsAuditDataSource(aopCaseEventsAuditList), 
                      columns: <DataColumn>[ 
                DataColumn( 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AopCaseEventsAuditCardList = 
        aopCaseEventsAuditList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AopCaseEventsAuditCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AopCaseEventsAudit e) { 
...
Widget _buildListView() {  
    ...
    if (aopCaseEventsAuditList.length == 0) { 
    return Column( 
      children: [ 
    ...
    var noOfFields = aopCaseEventsAuditList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            aopCaseEventsAuditList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(aopCaseEventsAuditList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(aopCaseEventsAuditList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: aopCaseEventsAuditList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AopCaseEventsAudit oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AopCaseEventsAudit Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AopCaseEventsAuditInfo(foundAopCaseEventsAudit: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AopCaseEventsAuditDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AopCaseEventsAuditEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AopCaseEventsAuditService.AopCaseEventsAuditDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  aopCaseEventsAuditList.sort((item1, item2) => 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AopCaseEventsAuditDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AopCaseEventsAuditDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AopCaseEventsAudit> _data; 
 
  AopCaseEventsAuditDataSource(List<AopCaseEventsAudit> aopCaseEventsAuditList) { 
    _data = aopCaseEventsAuditList; 
  } 
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AopCaseEventsAuditInfo(foundAopCaseEventsAudit: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AopCaseEventsAuditDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AopCaseEventsAuditInfo extends StatelessWidget { 	
	  final AopCaseEventsAudit foundAopCaseEventsAudit; 	
	  const _AopCaseEventsAuditInfo({Key? key, required this.foundAopCaseEventsAudit}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AopCaseEventsAudit_dispForm(context, foundAopCaseEventsAudit); 	
	  } 	
	} 	
