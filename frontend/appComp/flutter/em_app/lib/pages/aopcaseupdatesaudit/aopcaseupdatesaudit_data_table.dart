...
import 'package:em_app/pages/aopcaseupdatesaudit/aopcaseupdatesaudit_service.dart'; 
import 'package:em_app/pages/aopcaseupdatesaudit/aopcaseupdatesaudit_lib.dart'; 
...
class AopCaseUpdatesAuditDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AopCaseUpdatesAuditDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AopCaseUpdatesAuditDataTableState createState() => _AopCaseUpdatesAuditDataTableState(); 
} 
 
class _AopCaseUpdatesAuditDataTableState extends State<AopCaseUpdatesAuditDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AopCaseUpdatesAudit Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<AopCaseUpdatesAudit> aopCaseUpdatesAuditList = []; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AopCaseUpdatesAudit List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AopCaseUpdatesAudit List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AopCaseUpdatesAuditSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AopCaseUpdatesAuditService.AopCaseUpdatesAuditSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
      ...
      setState(() { 
        aopCaseUpdatesAuditList = getList; 
        _loading = false; 
      }); 
      }  
    }); 
    ...
    print("Calling Service : AopCaseUpdatesAuditViewAll ... "); 
    AopCaseUpdatesAuditService.AopCaseUpdatesAuditViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AopCaseUpdatesAuditFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AopCaseUpdatesAuditService.AopCaseUpdatesAuditFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AopCaseUpdatesAuditDataSource(aopCaseUpdatesAuditList), 
                      columns: <DataColumn>[ 
                DataColumn( 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AopCaseUpdatesAuditCardList = 
        aopCaseUpdatesAuditList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AopCaseUpdatesAuditCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AopCaseUpdatesAudit e) { 
...
Widget _buildListView() {  
    ...
    if (aopCaseUpdatesAuditList.length == 0) { 
    return Column( 
      children: [ 
    ...
    var noOfFields = aopCaseUpdatesAuditList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            aopCaseUpdatesAuditList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(aopCaseUpdatesAuditList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(aopCaseUpdatesAuditList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: aopCaseUpdatesAuditList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AopCaseUpdatesAudit oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AopCaseUpdatesAudit Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AopCaseUpdatesAuditInfo(foundAopCaseUpdatesAudit: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AopCaseUpdatesAuditDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AopCaseUpdatesAuditEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AopCaseUpdatesAuditService.AopCaseUpdatesAuditDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  aopCaseUpdatesAuditList.sort((item1, item2) => 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AopCaseUpdatesAuditDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AopCaseUpdatesAuditDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AopCaseUpdatesAudit> _data; 
 
  AopCaseUpdatesAuditDataSource(List<AopCaseUpdatesAudit> aopCaseUpdatesAuditList) { 
    _data = aopCaseUpdatesAuditList; 
  } 
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AopCaseUpdatesAuditInfo(foundAopCaseUpdatesAudit: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AopCaseUpdatesAuditDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AopCaseUpdatesAuditInfo extends StatelessWidget { 	
	  final AopCaseUpdatesAudit foundAopCaseUpdatesAudit; 	
	  const _AopCaseUpdatesAuditInfo({Key? key, required this.foundAopCaseUpdatesAudit}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AopCaseUpdatesAudit_dispForm(context, foundAopCaseUpdatesAudit); 	
	  } 	
	} 	
