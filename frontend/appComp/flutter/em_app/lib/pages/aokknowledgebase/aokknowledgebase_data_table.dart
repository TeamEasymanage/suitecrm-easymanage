...
class AokKnowledgebaseDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AokKnowledgebaseDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AokKnowledgebaseDataTableState createState() => _AokKnowledgebaseDataTableState(); 
} 
 
class _AokKnowledgebaseDataTableState extends State<AokKnowledgebaseDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AokKnowledgebase Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<AokKnowledgebase> aokKnowledgebaseList = []; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AokKnowledgebase List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AokKnowledgebase List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AokKnowledgebaseSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AokKnowledgebaseService.AokKnowledgebaseSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : AokKnowledgebaseViewAll ... "); 
    AokKnowledgebaseService.AokKnowledgebaseViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AokKnowledgebaseFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AokKnowledgebaseService.AokKnowledgebaseFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AokKnowledgebaseDataSource(aokKnowledgebaseList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                ...
                DataColumn( 
                    label: const Text('Revision'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Additional Info'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('User Id C'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('User Id1 C'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AokKnowledgebaseCardList = 
        aokKnowledgebaseList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AokKnowledgebaseCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AokKnowledgebase e) { 
...
Widget _buildListView() {  
    ...
    if (aokKnowledgebaseList.length == 0) { 
    return Column( 
      children: [ 
    ...
    var noOfFields = aokKnowledgebaseList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            aokKnowledgebaseList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(aokKnowledgebaseList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(aokKnowledgebaseList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: aokKnowledgebaseList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AokKnowledgebase oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AokKnowledgebase Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AokKnowledgebaseInfo(foundAokKnowledgebase: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AokKnowledgebaseDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AokKnowledgebaseEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AokKnowledgebaseService.AokKnowledgebaseDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  aokKnowledgebaseList.sort((item1, item2) => 
	  ...
	  aokKnowledgebaseList.sort((item1, item2) => 
		  compareColumn(ascending, item1.revision, item2.revision)); 
	} else if (columnIndex == 11) { 
	  aokKnowledgebaseList.sort((item1, item2) => 
		  compareColumn(ascending, item1.additionalInfo, item2.additionalInfo)); 
	} else if (columnIndex == 12) { 
	  aokKnowledgebaseList.sort((item1, item2) => 
		  compareColumn(ascending, item1.userIdC, item2.userIdC)); 
	} else if (columnIndex == 13) { 
	  aokKnowledgebaseList.sort((item1, item2) => 
		  compareColumn(ascending, item1.userId1C, item2.userId1C)); 
    } 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AokKnowledgebaseDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AokKnowledgebaseDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AokKnowledgebase> _data; 
 
  AokKnowledgebaseDataSource(List<AokKnowledgebase> aokKnowledgebaseList) { 
    _data = aokKnowledgebaseList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].revision))), 
	DataCell(Text(getEmStrOpt(_data[index].additionalInfo))), 
	DataCell(Text(getEmStrOpt(_data[index].userIdC))), 
	DataCell(Text(getEmStrOpt(_data[index].userId1C))), 
      DataCell(Row( 	
        mainAxisAlignment: MainAxisAlignment.spaceBetween, 	
        children: [ 	
          Builder(builder: (context) { 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AokKnowledgebaseInfo(foundAokKnowledgebase: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AokKnowledgebaseDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AokKnowledgebaseInfo extends StatelessWidget { 	
	  final AokKnowledgebase foundAokKnowledgebase; 	
	  const _AokKnowledgebaseInfo({Key? key, required this.foundAokKnowledgebase}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AokKnowledgebase_dispForm(context, foundAokKnowledgebase); 	
	  } 	
	} 	
