...
class AclRolesUsersOnlyDd extends StatefulWidget { 
  const AclRolesUsersOnlyDd({Key? key}) : super(key: key); 
 
  @override 
  _AclRolesUsersOnlyDdState createState() => _AclRolesUsersOnlyDdState(); 
} 
 
class _AclRolesUsersOnlyDdState extends State<AclRolesUsersOnlyDd> { 
    ...
    AclRolesUsersService.AclRolesUsersViewAll( 	
      ...
      setState(() { 	
       aclRolesUsersList = getList; 	
		idSelected = aclRolesUsersList[0].toListLabVal()[0][1].toString();	
		 //Remove duplicates 	
        //aclRolesUsersList = aclRolesUsersList.toSet().toList();	
        _loading = false;	
      });	
    });	
  } 	
...
class AclRolesUsersEditDd extends StatefulWidget { 
  //need 1st record column and type also 
  final String idSelected; 
  const AclRolesUsersEditDd({Key? key, required this.idSelected}) : super(key: key); 
 
  @override 
  _AclRolesUsersEditDdState createState() => _AclRolesUsersEditDdState(); 
} 
 
class _AclRolesUsersEditDdState extends State<AclRolesUsersEditDd> { 
  ...
  void initState() {	
    ...
    AclRolesUsersService.AclRolesUsersViewAllIds().then((ids) { 	
      if (ids is String) {	
        setState(() {	
          errorMsg = ids;	
        });	
  ...
  Widget build(BuildContext context) { 	
    return Scaffold( 	
	     backgroundColor: Colors.blue.shade200,	
        appBar: AppBar( 	
          title: const Text("AclRolesUsers Edit Dd Form"), 	
          centerTitle: true, 	
        ), 	
        body: _loading 	
            ? EmShowProgress(msg: errorMsg)	
                ...
                child: Row(	
                  ...
                  children: [	
                    Padding(	
                      ...
                      _isKeySelected 	
                          ? FutureBuilder<AclRolesUsers>( 	
                              future: AclRolesUsersService 	
                                      .AclRolesUsersQuery(_chosenValue) 	
                                  .then((value) => value), 	
                                ...
                                switch (snapshot.connectionState) { 	
                                  ...
                                  default: 	
                                    if (snapshot.hasError) { 	
                                      ...
                                      return AclRolesUsersEditDdRec( 	
                                          aclRolesUsers: snapshot.data!); 	
                                    } 	
                                } 	
                              }) 	
                          : const SizedBox( 	
...
class AclRolesUsersEditDdRec extends StatefulWidget { 
  final AclRolesUsers aclRolesUsers; 
  const AclRolesUsersEditDdRec({Key? key, required this.aclRolesUsers}) : super(key: key); 
 
  @override 
  _AclRolesUsersEditDdRecState createState() => _AclRolesUsersEditDdRecState(); 
} 
 
class _AclRolesUsersEditDdRecState extends State<AclRolesUsersEditDdRec> { 
  AclRolesUsers foundAclRolesUsers = AclRolesUsers(id: "" ); 
...
void didChangeDependencies() { 
  ...
  void initState() { 
    super.initState(); 
    setState(() { 
      foundAclRolesUsers = widget.aclRolesUsers; 
    }); 
  } 
            ...
            child: Form( 
              ...
              child: Padding( 
				...
				Flexible( 
					  child:  
				  TextFormField( 
					   enabled: false,    initialValue: getEmStrOpt(foundAclRolesUsers.id),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
				...
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAclRolesUsers.roleId),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
                  ...
                  const SizedBox(height: 10), 
				  // ---------------------------------------------------------------------------------------  
				  // Column Details:  Srl No : [3] [ Name [Userid] : EM Type [CHAR], Java Type [String]  
				  //                  EM size: maxlength: 36, Display Size : 20 
				  // Flutter Details: Field Label [User Id], Name [userId], Type [String]  
				  // ---------------------------------------------------------------------------------------  
				Row( 
				  mainAxisAlignment: MainAxisAlignment.spaceBetween, 
				  children: <Widget>[ 
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAclRolesUsers.userId),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
					...
					*/  
					//maxLength: 30, 
					onSaved: (value) => setState(() => userIdVar = value.toString()), 
				  ), //textformfield 
				), //flexible 
				]), //Widget[] //Row 
                  const SizedBox(height: 10), 
				...
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAclRolesUsers.dateModified),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
				...
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAclRolesUsers.deleted),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
                    ...
                    builder: (context) => ButtonWidget( 
                      ...
                      onClicked: () async { 
						...
						// ===== BEGIN BLOCK : Calling API Service Calls ('Edit') 1st (optional 2nd...) and Routing to Next Screen 
                          ...
                          AclRolesUsers newAclRolesUsers = AclRolesUsers( 
					   id: idVar , 
					   roleId: roleIdVar , 
					   userId: userIdVar , 
					   ...
					   deleted: deletedVar , 
                          ...
                          await AclRolesUsersService.AclRolesUsersEdit( 
                                  newAclRolesUsers.id, 
                                  newAclRolesUsers) 
                              .then((message) { 
                                    apiMessage = message; 
