...
class AccountsBugsOnlyDd extends StatefulWidget { 
  const AccountsBugsOnlyDd({Key? key}) : super(key: key); 
 
  @override 
  _AccountsBugsOnlyDdState createState() => _AccountsBugsOnlyDdState(); 
} 
 
class _AccountsBugsOnlyDdState extends State<AccountsBugsOnlyDd> { 
    ...
    AccountsBugsService.AccountsBugsViewAll( 	
      ...
      setState(() { 	
       accountsBugsList = getList; 	
		idSelected = accountsBugsList[0].toListLabVal()[0][1].toString();	
		 //Remove duplicates 	
        //accountsBugsList = accountsBugsList.toSet().toList();	
        _loading = false;	
      });	
    });	
  } 	
...
class AccountsBugsEditDd extends StatefulWidget { 
  //need 1st record column and type also 
  final String idSelected; 
  const AccountsBugsEditDd({Key? key, required this.idSelected}) : super(key: key); 
 
  @override 
  _AccountsBugsEditDdState createState() => _AccountsBugsEditDdState(); 
} 
 
class _AccountsBugsEditDdState extends State<AccountsBugsEditDd> { 
  ...
  void initState() {	
    ...
    AccountsBugsService.AccountsBugsViewAllIds().then((ids) { 	
      if (ids is String) {	
        setState(() {	
          errorMsg = ids;	
        });	
  ...
  Widget build(BuildContext context) { 	
    return Scaffold( 	
	     backgroundColor: Colors.blue.shade200,	
        appBar: AppBar( 	
          title: const Text("AccountsBugs Edit Dd Form"), 	
          centerTitle: true, 	
        ), 	
        body: _loading 	
            ? EmShowProgress(msg: errorMsg)	
                ...
                child: Row(	
                  ...
                  children: [	
                    Padding(	
                      ...
                      _isKeySelected 	
                          ? FutureBuilder<AccountsBugs>( 	
                              future: AccountsBugsService 	
                                      .AccountsBugsQuery(_chosenValue) 	
                                  .then((value) => value), 	
                                ...
                                switch (snapshot.connectionState) { 	
                                  ...
                                  default: 	
                                    if (snapshot.hasError) { 	
                                      ...
                                      return AccountsBugsEditDdRec( 	
                                          accountsBugs: snapshot.data!); 	
                                    } 	
                                } 	
                              }) 	
                          : const SizedBox( 	
...
class AccountsBugsEditDdRec extends StatefulWidget { 
  final AccountsBugs accountsBugs; 
  const AccountsBugsEditDdRec({Key? key, required this.accountsBugs}) : super(key: key); 
 
  @override 
  _AccountsBugsEditDdRecState createState() => _AccountsBugsEditDdRecState(); 
} 
 
class _AccountsBugsEditDdRecState extends State<AccountsBugsEditDdRec> { 
  AccountsBugs foundAccountsBugs = AccountsBugs(id: "" ); 
...
void didChangeDependencies() { 
  ...
  void initState() { 
    super.initState(); 
    setState(() { 
      foundAccountsBugs = widget.accountsBugs; 
    }); 
  } 
            ...
            child: Form( 
              ...
              child: Padding( 
				...
				Flexible( 
					  child:  
				  TextFormField( 
					   enabled: false,    initialValue: getEmStrOpt(foundAccountsBugs.id),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
				...
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAccountsBugs.accountId),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
                  ...
                  const SizedBox(height: 10), 
				  // ---------------------------------------------------------------------------------------  
				  // Column Details:  Srl No : [3] [ Name [Bugid] : EM Type [CHAR], Java Type [String]  
				  //                  EM size: maxlength: 36, Display Size : 20 
				  // Flutter Details: Field Label [Bug Id], Name [bugId], Type [String]  
				  // ---------------------------------------------------------------------------------------  
				Row( 
				  mainAxisAlignment: MainAxisAlignment.spaceBetween, 
				  children: <Widget>[ 
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAccountsBugs.bugId),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
					...
					*/  
					//maxLength: 30, 
					onSaved: (value) => setState(() => bugIdVar = value.toString()), 
				  ), //textformfield 
				), //flexible 
				]), //Widget[] //Row 
                  const SizedBox(height: 10), 
				...
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAccountsBugs.dateModified),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
				...
				Flexible( 
					  child:  
				  TextFormField( 
					      initialValue: getEmStrOpt(foundAccountsBugs.deleted),   
					  /* 
					  */ 
					  /* 
					  inputFormatters: [ 
                    ...
                    builder: (context) => ButtonWidget( 
                      ...
                      onClicked: () async { 
						...
						// ===== BEGIN BLOCK : Calling API Service Calls ('Edit') 1st (optional 2nd...) and Routing to Next Screen 
                          ...
                          AccountsBugs newAccountsBugs = AccountsBugs( 
					   id: idVar , 
					   ...
					   deleted: deletedVar , 
                          ...
                          await AccountsBugsService.AccountsBugsEdit( 
                                  newAccountsBugs.id, 
                                  newAccountsBugs) 
                              .then((message) { 
                                    apiMessage = message; 
