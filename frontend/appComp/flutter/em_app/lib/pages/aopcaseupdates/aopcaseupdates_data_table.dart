...
import 'package:em_app/pages/aopcaseupdates/aopcaseupdates_model.dart'; 
import 'package:em_app/pages/aopcaseupdates/aopcaseupdates_service.dart'; 
import 'package:em_app/pages/aopcaseupdates/aopcaseupdates_lib.dart'; 
...
class AopCaseUpdatesDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AopCaseUpdatesDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AopCaseUpdatesDataTableState createState() => _AopCaseUpdatesDataTableState(); 
} 
 
class _AopCaseUpdatesDataTableState extends State<AopCaseUpdatesDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AopCaseUpdates Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<AopCaseUpdates> aopCaseUpdatesList = []; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AopCaseUpdates List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AopCaseUpdates List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AopCaseUpdatesSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AopCaseUpdatesService.AopCaseUpdatesSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : AopCaseUpdatesViewAll ... "); 
    AopCaseUpdatesService.AopCaseUpdatesViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AopCaseUpdatesFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AopCaseUpdatesService.AopCaseUpdatesFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AopCaseUpdatesDataSource(aopCaseUpdatesList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                ...
                DataColumn( 
                    label: const Text('Case Id'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Contact Id'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Internal'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AopCaseUpdatesCardList = 
        aopCaseUpdatesList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AopCaseUpdatesCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AopCaseUpdates e) { 
...
Widget _buildListView() {  
    ...
    if (aopCaseUpdatesList.length == 0) { 
    return Column( 
      children: [ 
    ...
    var noOfFields = aopCaseUpdatesList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            aopCaseUpdatesList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(aopCaseUpdatesList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(aopCaseUpdatesList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: aopCaseUpdatesList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AopCaseUpdates oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AopCaseUpdates Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AopCaseUpdatesInfo(foundAopCaseUpdates: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AopCaseUpdatesDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AopCaseUpdatesEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AopCaseUpdatesService.AopCaseUpdatesDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  aopCaseUpdatesList.sort((item1, item2) => 
	  ...
	  aopCaseUpdatesList.sort((item1, item2) => 
		  compareColumn(ascending, item1.caseId, item2.caseId)); 
	} else if (columnIndex == 10) { 
	  aopCaseUpdatesList.sort((item1, item2) => 
		  compareColumn(ascending, item1.contactId, item2.contactId)); 
	} else if (columnIndex == 11) { 
	  aopCaseUpdatesList.sort((item1, item2) => 
		  compareColumn(ascending, item1.internal, item2.internal)); 
    } 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AopCaseUpdatesDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AopCaseUpdatesDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AopCaseUpdates> _data; 
 
  AopCaseUpdatesDataSource(List<AopCaseUpdates> aopCaseUpdatesList) { 
    _data = aopCaseUpdatesList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].caseId))), 
	DataCell(Text(getEmStrOpt(_data[index].contactId))), 
	DataCell(Text(getEmStrOpt(_data[index].internal))), 
      DataCell(Row( 	
        mainAxisAlignment: MainAxisAlignment.spaceBetween, 	
        children: [ 	
          Builder(builder: (context) { 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AopCaseUpdatesInfo(foundAopCaseUpdates: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AopCaseUpdatesDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AopCaseUpdatesInfo extends StatelessWidget { 	
	  final AopCaseUpdates foundAopCaseUpdates; 	
	  const _AopCaseUpdatesInfo({Key? key, required this.foundAopCaseUpdates}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AopCaseUpdates_dispForm(context, foundAopCaseUpdates); 	
	  } 	
	} 	
