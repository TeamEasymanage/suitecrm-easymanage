...
import 'package:em_app/pages/accountscstm/accountscstm_service.dart'; 
import 'package:em_app/pages/accountscstm/accountscstm_lib.dart'; 
...
class AccountsCstmDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AccountsCstmDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AccountsCstmDataTableState createState() => _AccountsCstmDataTableState(); 
} 
 
class _AccountsCstmDataTableState extends State<AccountsCstmDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AccountsCstm Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<AccountsCstm> accountsCstmList = []; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AccountsCstm List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AccountsCstm List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AccountsCstmSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AccountsCstmService.AccountsCstmSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : AccountsCstmViewAll ... "); 
    AccountsCstmService.AccountsCstmViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AccountsCstmFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AccountsCstmService.AccountsCstmFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AccountsCstmDataSource(accountsCstmList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                    label: const Text('Id C'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Jjwg Maps Lng C'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Jjwg Maps Lat C'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Jjwg Maps Geocode Status C'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Jjwg Maps Address C'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AccountsCstmCardList = 
        accountsCstmList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AccountsCstmCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AccountsCstm e) { 
...
Widget _buildListView() {  
    ...
    var noOfFields = accountsCstmList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            accountsCstmList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(accountsCstmList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(accountsCstmList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: accountsCstmList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AccountsCstm oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AccountsCstm Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AccountsCstmInfo(foundAccountsCstm: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  content: Container( 
                                    child: Column( 
                                      mainAxisSize: MainAxisSize.min, 
                                      children: [ 
                                        Text("Record Id: " +oh.idC.toString() ), 
                                        const Text(""), 
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      onPressed: () { 
                                        String valParam = ""; 
                                        if (oh.idC is String ) { 
                                          valParam = "'"+oh.idC.toString()+"'"; 
                                        } else { 
                                          valParam = oh.idC.toString(); 
                                        } 
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AccountsCstmDataTable(viewType: dispType, pageNo: 0, searchClause: " idC = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AccountsCstmEdit( 	 
                      				idCSelected: oh.idC, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AccountsCstmService.AccountsCstmDelete( 	 
                                      oh.idC) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  accountsCstmList.sort((item1, item2) => 
		  compareColumn(ascending, item1.idC, item2.idC)); 
	} else if (columnIndex == 1) { 
	  accountsCstmList.sort((item1, item2) => 
		  compareColumn(ascending, item1.jjwgMapsLngC, item2.jjwgMapsLngC)); 
	} else if (columnIndex == 2) { 
	  accountsCstmList.sort((item1, item2) => 
		  compareColumn(ascending, item1.jjwgMapsLatC, item2.jjwgMapsLatC)); 
	} else if (columnIndex == 3) { 
	  accountsCstmList.sort((item1, item2) => 
		  compareColumn(ascending, item1.jjwgMapsGeocodeStatusC, item2.jjwgMapsGeocodeStatusC)); 
	} else if (columnIndex == 4) { 
	  accountsCstmList.sort((item1, item2) => 
		  compareColumn(ascending, item1.jjwgMapsAddressC, item2.jjwgMapsAddressC)); 
    } 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AccountsCstmDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AccountsCstmDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AccountsCstm> _data; 
 
  AccountsCstmDataSource(List<AccountsCstm> accountsCstmList) { 
    _data = accountsCstmList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].idC))), 
	DataCell(Text(getEmStrOpt(_data[index].jjwgMapsLngC.toString()))), //DataCell(Text(getEmStrOpt(getEmFmtCurOpt(_data[index].jjwgMapsLngC)))), 
	DataCell(Text(getEmStrOpt(_data[index].jjwgMapsLatC.toString()))), //DataCell(Text(getEmStrOpt(getEmFmtCurOpt(_data[index].jjwgMapsLatC)))), 
	DataCell(Text(getEmStrOpt(_data[index].jjwgMapsGeocodeStatusC))), 
	DataCell(Text(getEmStrOpt(_data[index].jjwgMapsAddressC))), 
      DataCell(Row( 	
        mainAxisAlignment: MainAxisAlignment.spaceBetween, 	
        children: [ 	
          Builder(builder: (context) { 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AccountsCstmInfo(foundAccountsCstm: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AccountsCstmDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AccountsCstmInfo extends StatelessWidget { 	
	  final AccountsCstm foundAccountsCstm; 	
	  const _AccountsCstmInfo({Key? key, required this.foundAccountsCstm}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AccountsCstm_dispForm(context, foundAccountsCstm); 	
	  } 	
	} 	
