...
class AclRolesDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AclRolesDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AclRolesDataTableState createState() => _AclRolesDataTableState(); 
} 
 
class _AclRolesDataTableState extends State<AclRolesDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AclRoles Data Table"; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AclRoles List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AclRoles List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AclRolesSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AclRolesService.AclRolesSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : AclRolesViewAll ... "); 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AclRolesFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AclRolesService.AclRolesFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AclRolesDataSource(aclRolesList), 
                      columns: <DataColumn>[ 
                DataColumn( 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    ...
    return GridView.count( 
        ...
        children: List.from(AclRolesCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AclRoles e) { 
...
Widget _buildListView() {  
    ...
    var noOfFields = aclRolesList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            aclRolesList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(aclRolesList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(aclRolesList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: aclRolesList  
          .map((e) =>  
...
DataRow getDataRow(AclRoles oh) { 
                        ...
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AclRoles Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AclRolesInfo(foundAclRoles: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AclRolesDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AclRolesService.AclRolesDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AclRolesDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AclRolesDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AclRoles> _data; 
 
  AclRolesDataSource(List<AclRoles> aclRolesList) { 
    _data = aclRolesList; 
  } 
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AclRolesInfo(foundAclRoles: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AclRolesDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AclRolesInfo extends StatelessWidget { 	
	  final AclRoles foundAclRoles; 	
	  const _AclRolesInfo({Key? key, required this.foundAclRoles}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AclRoles_dispForm(context, foundAclRoles); 	
	  } 	
	} 	
