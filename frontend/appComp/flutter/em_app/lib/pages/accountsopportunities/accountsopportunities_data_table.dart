...
import 'package:em_app/pages/accountsopportunities/accountsopportunities_model.dart'; 
import 'package:em_app/pages/accountsopportunities/accountsopportunities_service.dart'; 
import 'package:em_app/pages/accountsopportunities/accountsopportunities_lib.dart'; 
...
class AccountsOpportunitiesDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AccountsOpportunitiesDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AccountsOpportunitiesDataTableState createState() => _AccountsOpportunitiesDataTableState(); 
} 
 
class _AccountsOpportunitiesDataTableState extends State<AccountsOpportunitiesDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AccountsOpportunities Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<AccountsOpportunities> accountsOpportunitiesList = []; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AccountsOpportunities List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AccountsOpportunities List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AccountsOpportunitiesSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AccountsOpportunitiesService.AccountsOpportunitiesSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
      ...
      setState(() { 
        accountsOpportunitiesList = getList; 
        _loading = false; 
      }); 
      }  
    }); 
    ...
    print("Calling Service : AccountsOpportunitiesViewAll ... "); 
    AccountsOpportunitiesService.AccountsOpportunitiesViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AccountsOpportunitiesFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AccountsOpportunitiesService.AccountsOpportunitiesFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AccountsOpportunitiesDataSource(accountsOpportunitiesList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                    label: const Text('Id'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Opportunity Id'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Account Id'), onSort: doColumnSort), 
                DataColumn( 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AccountsOpportunitiesCardList = 
        accountsOpportunitiesList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AccountsOpportunitiesCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AccountsOpportunities e) { 
...
Widget _buildListView() {  
    ...
    if (accountsOpportunitiesList.length == 0) { 
    return Column( 
      children: [ 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) =>  
                                  const AccountsOpportunitiesForm(), 	 
                                  ), 	 
                                ); 	 
                              }, 	 
                              icon: const Icon( 	 
    ...
    var noOfFields = accountsOpportunitiesList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            accountsOpportunitiesList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(accountsOpportunitiesList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(accountsOpportunitiesList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: accountsOpportunitiesList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(AccountsOpportunities oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AccountsOpportunities Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AccountsOpportunitiesInfo(foundAccountsOpportunities: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AccountsOpportunitiesDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AccountsOpportunitiesEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AccountsOpportunitiesService.AccountsOpportunitiesDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  accountsOpportunitiesList.sort((item1, item2) => 
		  compareColumn(ascending, item1.id, item2.id)); 
	} else if (columnIndex == 1) { 
	  accountsOpportunitiesList.sort((item1, item2) => 
		  compareColumn(ascending, item1.opportunityId, item2.opportunityId)); 
	} else if (columnIndex == 2) { 
	  accountsOpportunitiesList.sort((item1, item2) => 
		  compareColumn(ascending, item1.accountId, item2.accountId)); 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AccountsOpportunitiesDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AccountsOpportunitiesDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AccountsOpportunities> _data; 
 
  AccountsOpportunitiesDataSource(List<AccountsOpportunities> accountsOpportunitiesList) { 
    _data = accountsOpportunitiesList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].opportunityId))), 
	DataCell(Text(getEmStrOpt(_data[index].accountId))), 
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AccountsOpportunitiesInfo(foundAccountsOpportunities: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AccountsOpportunitiesDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AccountsOpportunitiesInfo extends StatelessWidget { 	
	  final AccountsOpportunities foundAccountsOpportunities; 	
	  const _AccountsOpportunitiesInfo({Key? key, required this.foundAccountsOpportunities}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AccountsOpportunities_dispForm(context, foundAccountsOpportunities); 	
	  } 	
	} 	
