...
class AmTasktemplatesOnlyDd extends StatefulWidget { 
  const AmTasktemplatesOnlyDd({Key? key}) : super(key: key); 
 
  @override 
  _AmTasktemplatesOnlyDdState createState() => _AmTasktemplatesOnlyDdState(); 
} 
 
class _AmTasktemplatesOnlyDdState extends State<AmTasktemplatesOnlyDd> { 
    ...
    AmTasktemplatesService.AmTasktemplatesViewAll( 	
      ...
      setState(() { 	
       amTasktemplatesList = getList; 	
		idSelected = amTasktemplatesList[0].toListLabVal()[0][1].toString();	
		 //Remove duplicates 	
        //amTasktemplatesList = amTasktemplatesList.toSet().toList();	
        _loading = false;	
      });	
    });	
  } 	
...
class AmTasktemplatesEditDd extends StatefulWidget { 
  //need 1st record column and type also 
  final String idSelected; 
  const AmTasktemplatesEditDd({Key? key, required this.idSelected}) : super(key: key); 
 
  @override 
  _AmTasktemplatesEditDdState createState() => _AmTasktemplatesEditDdState(); 
} 
 
class _AmTasktemplatesEditDdState extends State<AmTasktemplatesEditDd> { 
  ...
  Widget build(BuildContext context) { 	
    return Scaffold( 	
	     backgroundColor: Colors.blue.shade200,	
        appBar: AppBar( 	
          title: const Text("AmTasktemplates Edit Dd Form"), 	
          centerTitle: true, 	
        ), 	
        body: _loading 	
            ? EmShowProgress(msg: errorMsg)	
                                ...
                                switch (snapshot.connectionState) { 	
                                  ...
                                  default: 	
                                    if (snapshot.hasError) { 	
                                      ...
                                      return AmTasktemplatesEditDdRec( 	
...
class AmTasktemplatesEditDdRec extends StatefulWidget { 
  final AmTasktemplates amTasktemplates; 
  const AmTasktemplatesEditDdRec({Key? key, required this.amTasktemplates}) : super(key: key); 
 
  @override 
  _AmTasktemplatesEditDdRecState createState() => _AmTasktemplatesEditDdRecState(); 
} 
 
class _AmTasktemplatesEditDdRecState extends State<AmTasktemplatesEditDdRec> { 
