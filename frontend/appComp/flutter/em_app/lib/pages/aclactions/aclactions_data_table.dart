...
class AclActionsDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const AclActionsDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _AclActionsDataTableState createState() => _AclActionsDataTableState(); 
} 
 
class _AclActionsDataTableState extends State<AclActionsDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "AclActions Data Table"; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "AclActions List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "AclActions List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : AclActionsSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    AclActionsService.AclActionsSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : AclActionsViewAll ... "); 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : AclActionsFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    AclActionsService.AclActionsFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: AclActionsDataSource(aclActionsList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                ...
                DataColumn( 
                    label: const Text('Acltype'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Aclaccess'), onSort: doColumnSort), 
                DataColumn( 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable AclActionsCardList = 
        aclActionsList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(AclActionsCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(AclActions e) { 
...
Widget _buildListView() {  
    ...
    var noOfFields = aclActionsList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            aclActionsList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(aclActionsList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(aclActionsList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: aclActionsList  
          .map((e) =>  
...
DataRow getDataRow(AclActions oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('AclActions Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _AclActionsInfo(foundAclActions: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      ...
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            AclActionsDataTable(viewType: dispType, pageNo: 0, searchClause: " id = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => AclActionsEdit( 	 
                      				idSelected: oh.id, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await AclActionsService.AclActionsDelete( 	 
                                      oh.id) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  aclActionsList.sort((item1, item2) => 
	  ...
	  aclActionsList.sort((item1, item2) => 
		  compareColumn(ascending, item1.acltype, item2.acltype)); 
	} else if (columnIndex == 8) { 
	  aclActionsList.sort((item1, item2) => 
		  compareColumn(ascending, item1.aclaccess, item2.aclaccess)); 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  AclActionsDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class AclActionsDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<AclActions> _data; 
 
  AclActionsDataSource(List<AclActions> aclActionsList) { 
    _data = aclActionsList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].acltype))), 
	DataCell(Text(getEmStrOpt(getEmFmtNumOpt(_data[index].aclaccess)))), 
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _AclActionsInfo(foundAclActions: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const AclActionsDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _AclActionsInfo extends StatelessWidget { 	
	  final AclActions foundAclActions; 	
	  const _AclActionsInfo({Key? key, required this.foundAclActions}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return AclActions_dispForm(context, foundAclActions); 	
	  } 	
	} 	
